...
uses
  ...
  XMLSchema, WSDLPub, RpDefine, RpRave, RpCon, RpConDS, sButton, MPlayer,MMSystem;
...
type
  Tfrm_principal = class(TForm)
    ...
    sBevel6: TsBevel;
    sDragBar6: TsDragBar;
    btn_cancelar__: TsSpeedButton;
    btn_prog__: TsSpeedButton;
    btn_cancelar: TsButton;
    btn_prog: TsButton;
    sBevel7: TsBevel;
    sBevel8: TsBevel;
    VersoPortableVersionPortable1: TMenuItem;
    ...
    procedure btn_cancelar__Click(Sender: TObject);
    procedure btn_prog__Click(Sender: TObject);
    ...
    procedure edt_horaClick(Sender: TObject);
    procedure edt_minutosClick(Sender: TObject);
    procedure edt_horamsgClick(Sender: TObject);
    procedure edt_minutosmsgClick(Sender: TObject);
    ...
    procedure edt_minutosmsgChange(Sender: TObject);
    procedure edt_minutosChange(Sender: TObject);
    procedure edt_horamsgChange(Sender: TObject);
    procedure VersoPortableVersionPortable1Click(Sender: TObject);
  private
...
procedure Tfrm_principal.btn_prog__Click(Sender: TObject);
var
shut,shutmsg:string;
begin
...
TrayIcon1.BalloonHint:='iDisconnect esta em Ação';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='iDisconnect esta em Ação';
...
Tfrm_opcoes.SetIdioma( Tfrm_opcoes.GetIdioma, Self );
//btn_prog.ImageIndex:=4;
TrayIcon1.BalloonHint:='iDisconnect this Action';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='iDisconnect this Action';
TrayIcon1.ShowBalloonHint;
TrayIconInitialize.ShowBalloonHint;
end;
end;
...
shut,shutmsg:string;
begin
Cliquis; shut:= (edt_hora.Text)+':'+(edt_minutos.Text)+':'+(edt_segundos.Text);
shutMSG:= (edt_horamsg.Text)+':'+(edt_minutosmsg.Text)+':'+(edt_segundos.Text);
...
begin
btn_prog.Caption:='Operação Programada|Operaction Programing';
btn_cancelar.Caption:='Cancelar Operação|Cancel Operaction';
Tfrm_opcoes.SetIdioma( Tfrm_opcoes.GetIdioma, Self );
btn_prog.ImageIndex:=6;
TrayIcon1.BalloonHint:='iDisconnect esta em Ação';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='iDisconnect esta em Ação';
TrayIcon1.ShowBalloonHint;
...
begin
Cliquis;if CheckBox1.Checked = true then
begin
...
begin
Cliquis;if CheckBox2.Checked = true then
begin
 // edt_horamsg.Enabled:=true;
//edt_minutosmsg.Enabled:=true;
//UpDown5.Enabled:=true;
...
procedure Tfrm_principal.CheckBox2KeyPress(Sender: TObject; var Key: Char);
begin
if key =#13 then
begin
...
procedure Tfrm_principal.Cliquis;
var dir:string;
begin
...
begin
 sndPlaySound(pwidechar(dir+'.\sound\click.wav'),
...
procedure Tfrm_principal.ComboBox1Click(Sender: TObject);
begin
Cliquis;
end;
...
begin
cliquis;{if (   ( (edt_hora.text) > ('23'))         ) then
begin
if frm_opcoes.GetIdioma = 0 then
...
procedure Tfrm_principal.edt_horaClick(Sender: TObject);
begin
Cliquis;edt_hora.Clear;
edt_hora.SetFocus;
...
procedure Tfrm_principal.edt_horamsgChange(Sender: TObject);
begin
cliquis;
end;

procedure Tfrm_principal.edt_horamsgClick(Sender: TObject);
begin
Cliquis;edt_horamsg.Clear;
...
procedure Tfrm_principal.edt_minutosChange(Sender: TObject);
begin
cliquis;
end;

procedure Tfrm_principal.edt_minutosClick(Sender: TObject);
begin
Cliquis; edt_minutos.Clear;
//edt_minutos.SetFocus;
end;
...
procedure Tfrm_principal.edt_minutosmsgChange(Sender: TObject);
begin
Cliquis;
end;

procedure Tfrm_principal.edt_minutosmsgClick(Sender: TObject);
begin
edt_minutosmsg.Clear;
edt_minutosmsg.SetFocus;
end;
...
begin
Cliquis;{DM.tbl_operaction.Close;
...
RegisterHotKey(Handle, 1, MOD_CONTROL,  Ord('#13'));
...
var dir:string;
begin
dir:=ExtractFilePath(Application.ExeName)+'portable';



if DirectoryExists(dir) then
    frm_principal.VersoPortableVersionPortable1.Visible:=true
    else
    frm_principal.VersoPortableVersionPortable1.Visible:=false;
...
procedure Tfrm_principal.Memo1KeyPress(Sender: TObject; var Key: Char);
begin
{WITH frm_principal do
begin
if key = #13 then
...
if btn_prog.Enabled = true then
begin

if frm_opcoes.GetIdioma = 0 then
begin
TrayIcon1.BalloonHint:='iDisconnect esta iniciado';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='iDisconnect esta iniciado';
end
else
if frm_opcoes.GetIdioma = 1 then
begin
TrayIcon1.BalloonHint:='iDisconnect this started';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='iDisconnect this started';
end;
TrayIconInitialize.Visible:=FALSE;
Hide;
end
...
begin
if frm_opcoes.GetIdioma = 0 then
messagebox(handle,'O iDisconnect ainda não possui uma senha cadastrada para realizar a troca.'+#13+'Por Favor crie uma senha no Menu Opções.','Aviso', MB_ICONINFORMATION)
else
if frm_opcoes.GetIdioma = 1 then
messagebox(handle,'The iDisconnect does not have a password registered for replacement.'+#13+'Please create a password in Options Menu.',' Warning!', MB_ICONINFORMATION)
end;
...
begin
Cliquis;{if (DM.tbl_operaction.FieldByName('password').Asstring = '')and
...
SL := TStringList.Create;
  try
 // if not FileExists(dir + '.\doc\ptbr-help.htm') then
    SL.LoadFromFile(dir+ '.\doc\ptbr-help.htm');
    for I := 0 to SL.Count - 1 do
    begin
      ShowMessage(SL[I]);
      // Interage pelas linhas do arquivo.
    end;
  finally
    SL.Free;
  end;}
...
if not FileExists(dir + '.\help\help.chm') then
begin
if frm_opcoes.GetIdioma = 0 then
MessageBox(handle,'Documentação Corrompida!','Aviso',MB_ICONINFORMATION)
else
if frm_opcoes.GetIdioma = 1 then
MessageBox(handle,'Documentation Corrompted!','Warnning',MB_ICONINFORMATION);
end
else

begin
// abre a ajuda no tópico
   HelpFile:= ExtractFilePath(Application.ExeName) + '.\help\help.chm';
...
procedure Tfrm_principal.btn_cancelar__Click(Sender: TObject);
begin
...
if frm_opcoes.GetIdioma = 0 then
begin
TrayIcon1.BalloonHint:='A Operação do iDisconnect foi Cancelada';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='A operação do iDisconnect esta Cancelada';
end
else
if frm_opcoes.GetIdioma = 1 then
begin
TrayIcon1.BalloonHint:='Operation iDisconnect was Canceled';
TrayIcon1.BalloonTitle:='iDisconnect';
TrayIcon1.Hint:='The operation of this iDisconnect Canceled';
end;
TrayIcon1.Visible:=true;
...
procedure Tfrm_principal.Arquivo1Click(Sender: TObject);
begin
Cliquis;
end;
...
frm_principal.Show;
 ...
 if (CheckBox1.Enabled = true) and (btn_prog.Enabled = true) then
begin
 btn_cancelar.Enabled:=false;
btn_cancelar.Enabled:=true;
 btn_cancelar.Enabled:=false;
 ...
 if (CheckBox1.Enabled = false) and (btn_prog.Enabled = false) then
begin
...
procedure Tfrm_principal.VersoPortableVersionPortable1Click(Sender: TObject);
var dir :string;
begin
dir:= ExtractFilePath(Application.ExeName)+'portable';
if DirectoryExists(dir) then
    execExplorer(dir, True,True);
    //ou usar esse comando ShellExecute(Application.Handle,PChar('open'),PChar('explorer.exe'),PChar(dir),nil,SW_NORMAL);
